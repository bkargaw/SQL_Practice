...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
  SELECT
    count(name) , sum(case when mobile is null then 0 else 1 end)
  FROM
    teachers
  SQL
end
...
def teachers_and_divisions
  ...
  execute(<<-SQL)
  SELECT
    teachers.name , (case depts.id when 1 then 'Sci' when  2 then 'Sci' else  'Art' end)
  FROM
    teachers
  LEFT JOIN depts on depts.id = dept_id
  SQL
...
def teachers_and_divisions_two
  ...
  execute(<<-SQL)
  SELECT
    teachers.name , (case depts.id when 1 then 'Sci' when  2 then 'Sci' when 3 then 'Art' else  'None' end)
  FROM
    teachers
  LEFT JOIN depts on depts.id = dept_id
  SQL
